...
- `POST /maps/{mapId}/runs` — require_on_map("recommend.request")(실제 액션 판정, mapId가
  경로에 있다).
- run 하위 전부(evidence/regions/execute/result/widen/retry) — recommend/loaders.py::load_run +
...
def get_evidence(gated=RunGate, db: Session = DbSession):
    run, principal = gated
...
def patch_evidence(body: EvidencePatchRequest, gated=RunGate, db: Session = DbSession):
    run, principal = gated
