...
이전엔 maps/pins/shortlist가 각자 `def get_db_session(): yield from get_db()`를 복붙해뒀고
(authz는 그중 maps 걸 가져다 씀). 몸통이 같아도 FastAPI의 의존성 캐시는 함수 "객체" 동일성으로
재사용 여부를 판단하므로, 한 요청 안에서 서로 다른 모듈의 get_db_session이 같이 걸리면(예:
pins 라우터 + authz.guard의 멤버십 확인) DB 세션이 2개 열려 "요청 하나 = 트랜잭션 하나"가
깨졌다. 이제 공용 `common.database.get_db_session` 하나만 있고 각 모듈은 재노출만 한다 — 이
파일은 그 결과를 확인한다: (1) 네 모듈이 진짜 같은 함수 객체를 쓰는지, (2) 한 요청 안에서 그
함수가 여러 번 Depends로 걸려도 세션 인스턴스가 하나만 생기는지(FastAPI의 요청 스코프 의존성
캐싱).

auth는 아직 develop에 없어(#4, PR #92 미머지) 여기 대상에서 뺐다 — 그 PR이 머지되면
auth.deps.get_db_session도 같은 방식으로 추가해야 한다.
...
def test_four_modules_reexport_the_same_function_object():
    """maps/pins/shortlist/authz의 deps.py가 각자 정의를 두지 않고 재노출만 하는지 —
