...
(GitHub #16~21). 핀은 모양 3종(확정=채운 핀/일반=실선/AI 추천=점선)으로만 구분한다.
구성원별 색 구분은 없다 — #26에서 폐기됐고 `memberships.color` 필드도 없다. "누가 찍었는지"는
핀 상세의 `Pin.created_by`로 보여주고, 구성원별 보기는 `created_by` 필터로 한다.
